...
function MapView() {
  ...
  const currentLocation = useSearchStore((state) => state.currentLocation);

  useEffect(() => {
    if (!containerRef.current) return;

    // SDK 미로드 방어 — window 경유로 런타임 존재 여부 확인
    ...
    try {
      kakao.maps.load(() => {
        ...
        const seoulBounds = new kakao.maps.LatLngBounds(new kakao.maps.LatLng(SEOUL_SW.lat, SEOUL_SW.lng), new kakao.maps.LatLng(SEOUL_NE.lat, SEOUL_NE.lng));

        map.setMaxLevel(9);

        kakao.maps.event.addListener(map, 'dragend', () => {
          const pos = map.getCenter();
          if (!seoulBounds.contain(pos)) {
            map.panTo(center);
          }
        });
  ...
  return <div ref={containerRef} className="w-full h-full" aria-label="서울 상권 분석 지도" />;
}
